...
func renderBody(c *postContext) template.HTML {
...
func (c *postContext) parseTerminatedLine(line []byte) {
...
func (c *postContext) initLine(first byte) {
...
func (c *postContext) parseSpoilers(frag []byte, fn func([]byte)) {
	for {
...
func (c *postContext) parseFragment(frag []byte) {
...
func (c *postContext) parsePostLink(m [][]byte) {
	if c.Links == nil {
		c.Write(m[0])
		return
	}
	...
	t := renderPostLink(id, verified.OP, verified.Board, verified.OP != c.OP)
	c.WriteString(string(t))
}

// Parse internal or customly set reference URL
func (c *postContext) parseReference(m [][]byte) {
	var (
		m2   = string(m[2])
		href string
	)
...
func (c *postContext) newTabLink(href, text string) {
	fmt.Fprintf(
		c,
...
func (c *postContext) parseURL(bit []byte) {
	s := string(bit)
	switch {
...
func (c *postContext) escape(bit []byte) {
	c.WriteString(html.EscapeString(string(bit)))
}

// Parse a hash command
func (c *postContext) parseCommands(bit string) {
...
func (c *postContext) writeInvalidCommand(bit string) {
	c.WriteByte('#')
	c.WriteString(bit)
}

// Close any open HTML tags
func (c *postContext) terminateTags(newLine bool) {
	if c.state.spoiler {
		c.WriteString("</del>")
	}
	if c.state.quote {
