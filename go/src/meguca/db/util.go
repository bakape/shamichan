...
func IncrementPcount(tx *sql.Tx, b string) (c uint64, err error) {
	err = getStatement(tx, "increment_pcount").QueryRow(b).Scan(&c)
	return
}

// SetPcount sets the board's pyu counter. Only used in tests.
func SetPcount(c uint64) error {
	_, err := prepared["set_pcount"].Exec(c)
	return err
}
...
func WritePyuLimit(tx *sql.Tx, ip string, b string) error {
	_, err := getStatement(tx, "write_pyu_limit").Exec(ip, b, time.Now().Add(-time.Second).UTC())
	return err
}

// PyuLimitExists checks whether an IP has a pyu limit counter
func PyuLimitExists(tx *sql.Tx, ip string, b string) (exists bool, err error) {
	err = getStatement(tx, "pyu_limit_exists").QueryRow(ip, b).Scan(&exists)
	return
}
...
func GetPyuLimit(tx *sql.Tx, ip string, b string) (c uint8, err error) {
	err = getStatement(tx, "get_pyu_limit").QueryRow(ip, b).Scan(&c)
	return
}
...
func GetPyuLimitExpires(tx *sql.Tx, ip string, b string) (t time.Time, err error) {
	err = getStatement(tx, "get_pyu_limit_expires").QueryRow(ip, b).Scan(&t)
	return t.UTC(), err
}
...
func SetPyuLimitExpires(tx *sql.Tx, ip string, b string) error {
	_, err := getStatement(tx, "set_pyu_limit_expires").Exec(time.Now().Add(time.Hour).UTC(), ip, b)
	return err
}
