...
* Changed
  - JS injections now execute on all thread's and board's sections, articles and post forms
...
* Added
  ...
  - `scripts/send` for pushing arbitrary messages to all client from the server's shell
...
* Changed
  ...
  - Moved r/a/dio API polling server-side. Now passed to the clients through push notifications
  ...
  - Vagrant now supplies all of the dependancies, including optional. Be sure to run `vagrant provision` for existing VMs
* Removed
  - Deployment script
...
* Changed
  ...
  - Now using [io.js](https://iojs.org) instead of node.js. At the moment of writing should still be compatible with node.js v0.12.x. When upgrading be sure to run `make clean; npm update; npm install` from meguca's root directory.
...
## 0.4.0 - 2014-12-3
*  Info
  - Started versioning seperately from [doushio](https://github.com/lalcmellkmal/doushio)
